...
from . import ROOT
from .analysis import analyze, from_rows
...
def describe_session(data, *, source, label=None, source_sha256=None, source_kind='unverified'):
    ...
    for row in rows:
        ...
        if any(isinstance(row.get(k), bool) or not isinstance(row.get(k), (int, float))
               or not math.isfinite(row[k]) for k in ('timestamp', 'rssi_dbm')):
            raise ValueError('Valores no finitos o no numéricos')
        if not -120 <= row['rssi_dbm'] <= -1:
